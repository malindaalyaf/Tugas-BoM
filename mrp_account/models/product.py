...
class ProductProduct(models.Model):
    ...
    # Ambil function bom dari mrp_bom
    def _bom_cost_test(self):
        self.ensure_one()
        bom = self.env['mrp.bom']._bom_find(product=self)
        if bom:
            return bom["bom_cost_test"]
        return 0

    def _compute_bom_price(self, bom, boms_to_recompute=False):
        self.ensure_one()
        print("try compute bom price") #tes debug
        if not bom:
            return 0
        ...
        bomcost = self._bom_cost_test()
        return bom.product_uom_id._compute_price(total / bom.product_qty, self.uom_id) + bomcost
